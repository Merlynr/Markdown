...
**理解**：在函数体makeAdder()中返回了一个值，其中值得组成由三部分组成，其中包括变量add5给函数传得参数，及函数体内自己变量，还有add5(num)获取返回对象得时候进行传得参数。

==定义==：是一种规范类型，基于 ECMAScript 代码的词法嵌套结构来定义标识符和具体变量和函数的关联。一个词法环境由环境记录器和一个可能的引用外部词法环境的空值组成。
